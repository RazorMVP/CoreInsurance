...
@RestController
@RequestMapping("/api/v1/setup/adjusters")
@Tag(name = "Setup — Adjusters", description = "NAICOM-licensed loss-adjuster master data. Distinct from surveyors (pre-loss inspections); adjusters perform post-loss claim assessment.")
...
public class AdjusterController {
    ...
    @Operation(summary = "List adjusters (paginated)")
    @ApiResponses({
        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "Adjuster page",
            content = @Content(schema = @Schema(implementation = AdjusterResponse.class))),
        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "401", description = "Unauthorized", content = @Content),
        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "403", description = "Forbidden", content = @Content)
    })
    public ResponseEntity<ApiResponse<List<AdjusterResponse>>> list(
            @PageableDefault(size = 20) Pageable pageable) {
        Page<AdjusterResponse> page = service.list(pageable);
    ...
    @GetMapping("/{id}")
    @PreAuthorize("hasRole('SETUP_VIEW')")
    @Operation(summary = "Get adjuster by id")
    @ApiResponses({
    ...
    public ResponseEntity<ApiResponse<AdjusterResponse>> get(@PathVariable UUID id) {
        return ResponseEntity.ok(ApiResponse.success(service.get(id)));
    }

    @PostMapping
    @PreAuthorize("hasRole('SETUP_CREATE')")
    @Operation(summary = "Create an adjuster")
    @ApiResponses({
    ...
    public ResponseEntity<ApiResponse<AdjusterResponse>> create(
            @Valid @RequestBody AdjusterRequest request) {
    ...
    @Operation(summary = "Update adjuster")
    @ApiResponses({
    ...
    public ResponseEntity<ApiResponse<AdjusterResponse>> update(
            @PathVariable UUID id, @Valid @RequestBody AdjusterRequest request) {
        return ResponseEntity.ok(ApiResponse.success(service.update(id, request)));
    }

    @DeleteMapping("/{id}")
    @PreAuthorize("hasRole('SETUP_DELETE')")
    @Operation(summary = "Soft-delete adjuster")
    @ApiResponses({
