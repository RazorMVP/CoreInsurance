...
@RestController
@RequestMapping("/api/v1/setup/banks")
@Tag(name = "Setup — Banks", description = "Bank master data. Drives the bank selector on Receipt + Payment processing (Module 8).")
...
public class BankController {
    ...
    @Operation(summary = "List banks (paginated)")
    @ApiResponses({
        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "Bank page",
    ...
    public ResponseEntity<ApiResponse<List<BankResponse>>> list(
        ...
        Page<BankResponse> page = service.list(pageable);
    ...
    public ResponseEntity<ApiResponse<BankResponse>> get(@PathVariable UUID id) {
    ...
    public ResponseEntity<ApiResponse<BankResponse>> update(
            @PathVariable UUID id, @Valid @RequestBody BankRequest request) {
        return ResponseEntity.ok(ApiResponse.success(service.update(id, request)));
    }

    @DeleteMapping("/{id}")
    @PreAuthorize("hasRole('SETUP_DELETE')")
    @Operation(summary = "Soft-delete bank")
    @ApiResponses({
