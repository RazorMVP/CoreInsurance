...
@Tag(name = "Setup — Access Groups", description = "RBAC access group master data. Each group aggregates module-level permissions ({module}:create / view / update / approve). Users inherit permissions through their assigned access group.")
...
public class AccessGroupController {
    ...
    @Operation(summary = "List access groups (paginated)")
    @ApiResponses({
        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "Access group page",
            content = @Content(schema = @Schema(implementation = AccessGroupResponse.class))),
    ...
    @GetMapping("/{id}")
    @PreAuthorize("hasRole('SETUP_VIEW')")
    @Operation(summary = "Get access group by id")
    @ApiResponses({
    ...
    @Operation(summary = "Create an access group")
    @ApiResponses({
    ...
    @Operation(summary = "Update access group")
    @ApiResponses({
    ...
    @Operation(summary = "Soft-delete access group")
    @ApiResponses({
