...
| Image CVE scan | In review | The backend image workflow now runs Trivy against high and critical CVEs and uploads SARIF. Working scans found Spring, Thymeleaf, Tomcat, Bouncy Castle, and PostgreSQL JDBC CVEs, so the backend was upgraded to Spring Boot `3.5.14`, Spring Security Web `6.5.10`, Thymeleaf `3.1.5.RELEASE`, Tomcat `10.1.54`, Bouncy Castle `1.84`, and PostgreSQL JDBC `42.7.11`; final release evidence requires a successful GitHub workflow run for the exact release image. |
