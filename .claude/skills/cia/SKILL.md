...
**Partner Portal SPA (Frontend Phase 3, `apps/partner`) — shipped Sub-project B:** dark-mode developer portal consuming the Sub-project A `/portal/**` token-handler BFF via cookie-session (browser holds no token/secret except the one-time rotate reveal); P1 Credentials, P2 API Explorer (verbatim try-it relay), P3 Webhooks, P5 Usage (real telemetry) live; demo-first mock layer until the `partner` realm is provisioned; P4 Sandbox tracked separately as `partner-portal-sandbox-epic`.
