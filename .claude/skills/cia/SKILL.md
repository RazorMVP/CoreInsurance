...
### Module 12 — Period-End Closures (in progress; lives inside `cia-finance`)
IFRS-17 PAA + IFRS-9 + NAICOM monthly recap foundations. **Phase 1 (GL Foundations) — shipped slices:** 1.1 V31 GL schema (`fiscal_year`, `fiscal_period`, `period_lock`, `journal_entry`, `journal_entry_line`, `posting_rule`, `chart_of_account`); 1.2 V32 COA seed (129 rows, 3-level hierarchy); 1.3 `ChartOfAccountService` (read-only); 1.4 `JournalEntryService` + `TrialBalanceService` (GATEWAY — every later slice posts through here); 1.5 `SubledgerPostingService` event listeners (PolicyApproved / EndorsementApproved / ClaimApproved / ClaimSettled / FacPremiumCeded → JE post); 1.6 `FiscalYearService` + period generation + lazy DAY resolver; **1.7 (this slice) `PeriodLockService` + Hibernate `PeriodLockInterceptor` with 5-business-day grace, opt-in via `LockableByPeriod` marker interface, structured 423 LOCKED error, request-scoped lookup cache, Type-2 SCD lock history, PeriodReopenedEvent → CFO notification.** **Future slices:** 1.7a/b (entity opt-in sweep), 1.7c (PPA workflow + tenant CFO config + Nigerian holiday calendar), 1.8 (retroactive JE backfill), 1.9 (CI reconciliation gate). Phase 2 (IFRS 17 measurement), Phase 3 (IFRS 9), Phase 4 (NAICOM submissions) are 4–6 weeks each.
