...
| `[x]` | Backend: cia-reports module | Maven module: domain entities (ReportDefinition, ReportPin, ReportAccessPolicy) + JSONB config (ReportConfig + AttributeConverter) + repositories + services (ReportRunnerService, ReportQueryBuilder, ReportCsvRenderer, ReportPdfRenderer) + ReportController (14 endpoints). `ReportQueryBuilder.BASE_QUERIES` switched from `Map.of` → `Map.ofEntries` (15 entries) + `BASE_QUERY_TAILS` map for aggregation suffixes (TRIAL_BALANCE only) + 3 new filter keys (`account_code`, `source_module`, `classification`). |
