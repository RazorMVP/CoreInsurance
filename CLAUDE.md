...
| `CIA_PARTNER_PORTAL_CLIENT_ID` | Confidential Keycloak client id for the Partner Portal BFF's token-handler OAuth2 flow (`cia.partner-portal.client-id`, default `cia-partner-portal`). The browser SPA never sees this client's secret or a Keycloak token — `PortalAuthController` (cia-partner-portal-bff) is the OAuth2 client; the browser only ever carries the opaque `cia_portal_session` cookie. | env |
