...
**Backend Helm chart (Slice B):** `deploy/helm/cia-backend/` deploys **cia-api only** — Deployment + Service + Ingress (`/api` + `/partner` **only** — `/actuator/**` is never publicly routed) + HPA (CPU 3→10 @70%) + PDB (`minAvailable: 2`) + ConfigMap + ServiceAccount. The 5 backing services (Postgres, Keycloak, Temporal, object storage, Redis) are **external** — endpoints supplied via the ConfigMap env (URLs) + a pre-existing `Secret` referenced by `values.existingSecret` (the chart never templates secret values; see `secret.example.yaml` for the key contract). Deployment is hardened: `runAsNonRoot`, `readOnlyRootFilesystem` + `/tmp` emptyDir, drop-all-caps, `RuntimeDefault` seccomp, startup/liveness/readiness probes on `/actuator/health/*`, `checksum/config` rollout trigger. **Resilience (go-live hardening, 2026-06-24):** `topologySpreadConstraints` (node + zone, `maxSkew=1`/`ScheduleAnyway`, labelSelector auto-injected — `k8s-topology-spread`); graceful termination as three coordinated parts — native `lifecycle.preStop.sleep` (endpoint deregisters first) → `server.shutdown=graceful` + `spring.lifecycle.timeout-per-shutdown-phase=30s` in `application-prod.yml` (drains in-flight HTTP + the embedded Temporal worker) → `terminationGracePeriodSeconds=90` (`k8s-termination-grace`); HPA `behavior` stabilization windows (60s up / 300s down, tames JVM-cold-start thrash — `hpa-scaling-behavior`); and a **`values.schema.json`** (draft-07) Helm validates on lint/template — `additionalProperties:false` on the structured objects catches typos like `hpa.minRepicas` at render time (`chart-values-schema`). **Backup/DR (`db-backup-dr`):** an **opt-in** logical-backup `CronJob` (`backup.enabled=false` by default — secondary to managed-Postgres PITR, hardened + least-privilege secret access) plus the DR runbook at [`docs-site/docs/operations/disaster-recovery.md`](docs-site/docs/operations/disaster-recovery.md) (PII-key custody, schema-per-tenant surgical restore, quarterly drill). A real prod deploy MUST set BOTH `SPRING_PROFILES_ACTIVE=prod` AND `CIA_DEPLOYMENT_ENVIRONMENT=production` (neither implies the other — `ProductionSafetyValidator` keys off the marker). Pin `image.tag` to a built commit SHA (the chart `required`-guards an empty tag). Validated in CI by `helm-chart.yml` — `helm lint` + `kubeconform -strict` (default + prod-example values) + a `kind` smoke that boots the real image against ephemeral Postgres (**Temporal intentionally absent** — the app boots Temporal-degraded by design, proven by `TemporalUnreachableBootIT`; the smoke points `TEMPORAL_HOST` at a non-existent host) and asserts `/actuator/health` 200. **Not in this slice (separate go-live step):** a live cluster, DNS / cert-manager issuer, the secret-store wiring, and the managed backing-service endpoints; plus a deploy-to-cluster CD step.
...
- **Partner Portal (`apps/partner`, Sub-project B):** a **separate** Vercel project (dark-mode Insurtech developer portal). CI: `.github/workflows/vercel-deploy-partner.yml` (preview on PR, prod on push to `main`, filtered to `cia-frontend/**`; mirrors `vercel-deploy-platform.yml`). `cia-frontend/apps/partner/vercel.json` carries `outputDirectory: dist` (app-relative — the workflow builds from `cia-frontend/apps/partner`, so the app's own `vercel.json` is the project root for that path; the dashboard's own git-integration build instead needs `apps/partner/dist`, per `DEPLOY.md`). Required GitHub secret: **`VERCEL_PARTNER_PROJECT_ID`** (`VERCEL_TOKEN` / `VERCEL_ORG_ID` shared with back-office + platform). Env: `VITE_API_BASE_URL` (+ `VITE_DEMO_MODE=true` on the public preview only, baked into the workflow like platform's). **Note it is cookie-session, not Bearer** — the browser holds no token/secret except the one-time rotate-secret reveal — so real (non-demo) mode requires **two** infra prerequisites, not one: (1) the deployed partner Vercel origin added to the BFF's `cia.partner-portal.allowed-origins` (`CIA_PARTNER_PORTAL_ALLOWED_ORIGINS`), and (2) since the BFF's `cia_portal_session` cookie is `SameSite=Strict` and `SameSite` is evaluated on the registrable domain, the SPA and BFF must share a registrable domain (e.g. `partner.cia.app` ↔ `api.cia.app`) **or** the cookie must be relaxed to `SameSite=None; Secure` — a `*.vercel.app` origin cannot send the cookie no matter how CORS is configured (tracked as backlog `partner-portal-samesite-cookie`). Both are infra settings, not code. **Step-by-step one-time-setup runbook: [`cia-frontend/apps/partner/DEPLOY.md`](cia-frontend/apps/partner/DEPLOY.md)**.
...
| 6 | Reinsurance | 17 | RI allocations, offer slips, credit notes, bordereaux; **inward FAC (V75)** — accept/renew/extend/cancel a ceding insurer's share, server-side guaranty document, premium receivable (DebitNote + `FAC_PREMIUM_ACCEPTED` GL), live `/api/v1/ri/fac-inwards` + FACTab Inward tab; **both inward + outward FAC on IFRS-17 PAA (V76–V79, `fac-ifrs17-paa-workstream`, merged 2026-08-18)** — premium earned over coverage via the shared `LrcEngine` (inward LRC liability `Cr 2210`→release `Dr 2210/Cr 4330`; outward reinsurance-held asset `Dr 5210/Cr 1410` with §65 commission-netting on confirm), per-contract derecognition, §103 movement disclosure |
...
| Inward Facultative | Managed in Reinsurance module — **built V75** (accept a ceding insurer's share of a risk → server-side guaranty document + premium receivable DebitNote; `/api/v1/ri/fac-inwards` create/renew/extend/cancel; no local policy (external risk), ceding company = insurers master data). **On IFRS-17 PAA since `fac-ifrs17-paa-workstream` (V76–V79, merged 2026-08-18):** premium is now **earned over coverage** via the shared `LrcEngine` — accept sets up the LRC liability (`Cr 2210`), periodic release `Dr 2210 / Cr 4330` straight-line; cancellation derecognises **per-contract**. Supersedes the v1 immediate-income posting. |
...
FAC inward (accepted)  → Debit Note      → Finance Receivables → Receipt   (V75 receivable leg; income now earned over coverage via IFRS-17 PAA — accept Cr 2210 LRC, release Dr 2210 / Cr 4330 — since V76–V79)
```
...
**Guard blind spot — list pages must bind the *summary* DTO their endpoint returns.** `check-dto-drift` compares field *sets* (`XDto` ↔ `XResponse`) but cannot see that a component binds the *wrong* DTO for its endpoint. Two list pages white-screened this way (`/policies`, `/quotation`; both fixed 2026-06): each was typed `ColumnDef<XDto>` / `useQuery<XDto[]>` against the **full** detail DTO while the list endpoint returns a lean `X`**`SummaryResponse`** that omits fields the cell renders (e.g. `totalSumInsured`) → `undefined.toLocaleString()` white-screen, invisible to TS because the page over-claimed its type. **Rule:** a list page MUST bind the summary type its endpoint actually returns (`PolicySummaryDto`, `QuoteSummaryDto`, `*ListItemResponse`), never the full detail DTO. The zod-validated fetch paths (`validatedGet`/`validatedList`) are structurally immune (a field mismatch surfaces as a clean React-Query error, not a cell crash); the un-validated raw `apiClient.get` list pages are the exposure.

**Cell-formatter convention (null-tolerance).** Money / date / status cells use the shared null-tolerant helpers in `apps/back-office/src/lib/format.ts` — `formatNaira` / `formatDate` / `formatTimestamp` / `formatEnumLabel` (each renders `—` for `null`/`undefined`) — so a cell reading a missing or nullable field degrades gracefully instead of white-screening. **Never inline** `(getValue() as number).toLocaleString()` or `(getValue() as string).replace(...)` in a cell — that is the exact `undefined.<method>()` crash the helpers exist to prevent.
...
- **The interceptor's lock-check lookups MUST run `flushMode=COMMIT`** (do NOT remove). `PeriodLockService.checkWrite` runs from inside `PeriodLockInterceptor.onSave`/`onFlushDirty` — i.e. *during* a Hibernate flush — and resolves the period + active lock via `FiscalPeriodRepository.findFirst…` + `PeriodLockRepository.findFirst…`; both carry `@QueryHints(@QueryHint(name = "org.hibernate.flushMode", value = "COMMIT"))`. Under the default AUTO mode the pre-read auto-flush **re-enters the in-flight `LockableByPeriod` entity** and throws a spurious `Found shared references to a collection` on any such entity owning an *uninitialised* `@OneToMany(orphanRemoval=true)` collection (`Endorsement.risks`; `Payment` writes with a loaded `Claim.documents` in-session) — rolling back the write. This silently broke **every endorsement approval + claim-settlement payment** until fixed 2026-06-30 (the `CHECKING` ThreadLocal guards recursion but not the autoflush). The lookups read reference data the in-flight write never modifies, so skipping the pre-read flush is safe. Complementary: the finance beneficiary resolvers use scalar **projections** (`findBeneficiaryView`) so they never drag a `Claim`/`Endorsement` (+ its collections) into a write session.
...
| `CIA_PARTNER_RATE_LIMIT_STORE` | Active `PartnerBucketStore` impl for the per-client rate-limit buckets (`cia.partner.rate-limit.store`) — `redis` (`RedisPartnerBucketStore`, TTL'd, distributed across replicas) / `in-memory` (`InMemoryPartnerBucketStore`, default, `matchIfMissing=true`, per-replica only). Not `bucket4j` (the declarative starter is disabled — see `PartnerRateLimitFilter`). | env |
| `REDIS_URL` | Redis connection (partner rate limiting) | env / vault |
| `CIA_PARTNER_USAGE_STORE` | Active `PartnerUsageRollupStore` impl for request telemetry rollups (`cia.partner-usage.store`) — `redis` (`RedisPartnerUsageRollupStore`, shared across replicas) / `in-memory` (`InMemoryPartnerUsageRollupStore`, default, `matchIfMissing=true`, single-instance only — powers `/portal` usage). | env |
...
| `CIA_PARTNER_PORTAL_REALM` | Name of the `partner` Keycloak realm holding Insurtech developer users (role `PARTNER_DEVELOPER`) (`cia.partner-portal.realm`, default `partner`). Bound identically by `PartnerPortalBootstrapProperties` (cia-api) and `PartnerPortalRealmProperties` (cia-auth) — keep the two in sync, mirroring the platform realm pattern. | env |
| `CIA_PARTNER_PORTAL_CLIENT_ID` | PUBLIC (not confidential) Keycloak client id for the Partner Portal BFF's token-handler OAuth2 flow (`cia.partner-portal.client-id`, default `cia-partner-portal`) — provisioned with no client secret + PKCE (S256) via `ensurePartnerPortalClient`/`desiredPartnerPortalClient` (`setPublicClient(true)`); the authorization-code exchange sends no `client_secret`. The token-handler security invariant is preserved by PKCE + the server-side, httpOnly session — not by client confidentiality. The browser SPA never sees a Keycloak token either way — `PortalAuthController` (cia-partner-portal-bff) is the OAuth2 client; the browser only ever carries the opaque `cia_portal_session` cookie. | env |
| `CIA_PARTNER_PORTAL_REDIRECT_URIS` | The BFF's own `/portal/auth/callback` endpoint — the value that actually registers the `cia-partner-portal` Keycloak client's redirect URI (`cia.partner-portal.redirect-uris`, CSV; default `http://localhost:8090/portal/auth/callback`). **Not** the SPA origin (that's `cia.partner-portal.app-url` / `CIA_PARTNER_PORTAL_ALLOWED_ORIGINS` below) — since the BFF, not the browser, is the OAuth2 client in the token-handler pattern, Keycloak's registered `redirect_uri` must equal what `PortalAuthController` actually sends at authorize/token time. | env |
| `CIA_PARTNER_PORTAL_ALLOWED_ORIGINS` | CSV of exact browser origins allowed to call `/portal/**` cross-origin with credentials (the session cookie) (`cia.partner-portal.allowed-origins`, bound on cia-auth's `PartnerPortalRealmProperties`). Default `http://localhost:5174` (the local partner Vite origin). `allowCredentials(true)` ⇒ no `*` wildcard — mirrors `CIA_CORS_ALLOWED_ORIGINS` for the internal `/api/**` policy. Wired into `PortalSecurityConfig`'s `portalCorsConfigurationSource` bean (cia-partner-portal-bff). `/partner/**` (M2M, OAuth2 client-credentials) stays deliberately CORS-free, unaffected by this. | env |
| `CIA_PARTNER_PORTAL_STORE` | `redis` / `in-memory` (default) — selects `PortalSessionStore` + `PortalLoginStateStore` impl (`cia.partner-portal.store`). `in-memory` is correct for single-instance dev/test; `redis` is required once the BFF runs multi-replica (session + CSRF state must be shared across replicas, not per-pod). Mirrors the shape of `CIA_PARTNER_RATE_LIMIT_STORE` / `CIA_PARTNER_USAGE_STORE` above. | env |
| `CIA_PARTNER_PORTAL_BOOTSTRAP_ENABLED` | Master switch for the gated `PartnerPortalBootstrapRunner` (provisions the `partner` realm + first partner-developer admin on boot). `false` by default — dev + the IT suite never provision. **Requires `KEYCLOAK_ADMIN_ENABLED=true`.** Mirrors `CIA_PLATFORM_BOOTSTRAP_ENABLED` / `CIA_TENANT_BOOTSTRAP_ENABLED`. | env |
| `CIA_PARTNER_PORTAL_BOOTSTRAP_ADMIN_USERNAME` / `CIA_PARTNER_PORTAL_BOOTSTRAP_ADMIN_EMAIL` / `CIA_PARTNER_PORTAL_BOOTSTRAP_ADMIN_TEMP_PASSWORD` | First partner-developer admin's credentials, consumed by `PartnerPortalBootstrapRunner` when bootstrap is enabled (`cia.partner-portal.bootstrap.admin-{username,email,temp-password}`). The temp-password is a secret (forces `UPDATE_PASSWORD` on first login; `@ToString.Exclude`d, never logged); the runner fails fast if it is blank while bootstrap is enabled. | env / vault |
...
| `[x]` | Claims Setup | Tabbed full CRUD (S3b, 2026-07-27): Reserve Categories + Nature/Cause of Loss (`{name,code}` list-CRUD; Cause picks its Nature FK), Notification Timelines (per-product `notificationDays` singleton upsert), Required Documents (per-product list, `mandatory` Switch + `documentType` enum Select) — wired to `/api/v1/setup/{claim-reserve-categories,nature-of-loss,cause-of-loss}` + nested `products/{id}/claim-{notification-timeline,document-requirements}` |
...
| `[x]` | Inward FAC | **Live (V75, 2026-07-15)** — FACTab Inward tab is a real DataTable over `GET /api/v1/ri/fac-inwards` (`validatedGet` + zod); "Add Inward FAC" → `AddInwardFACSheet` (POST create); status-gated row actions: "Renew" → `InwardFACActionSheet` mode=RENEW (`{coverFrom,coverTo}`, terms carry over — no amendable share/rate in v1), "Extend period" → mode=EXTEND (`{newCoverTo}`), "Cancel" → reason dialog (`{reason}`), "Download guaranty" → blob stream from `/{id}/document`. (Replaced the interim "coming soon" placeholder from the allow-mock sweep.) |
...
| Status | Build | Deliverables |
|---|---|---|
| `[x]` | **P1. Authentication** | client_id + scopes + rotate-secret-once |
| `[x]` | **P2. API Explorer** | verbatim try-it relay incl. 403/429 |
| `[x]` | **P3. Webhook Management** | register/list/delete + delivery summary |
| `[ ]` | **P4. Sandbox** | Out of scope — no backend sandbox exists; tracked as backlog `partner-portal-sandbox-epic` |
| `[x]` | **P5. Usage Dashboard** | real telemetry StatCards + Recharts |
...
| Phase 3 — Partner Portal | 5 | 4 | `[~]` 4/5 (P4 Sandbox is a separate epic) |
| **Total** | **21** | **20** | **95% complete** |
