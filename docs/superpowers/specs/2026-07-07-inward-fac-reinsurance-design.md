...
**Accounting-model decision (flagged for review):** v1 uses this **simple income posting**, matching how outward FAC actually posts today. The V32 chart also seeds inward LRC/LIC liability accounts (`2210`/`2220`) for a full **IFRS-17 PAA/LRC** treatment (premium → LRC liability, recognized over coverage), but that path needs an inward LRC-release engine (Module 12 PAA machinery) and is **out of scope for v1** — and outward FAC is likewise not PAA-integrated. If the accounting team requires PAA treatment from day one, this is the section to revisit before implementation.
