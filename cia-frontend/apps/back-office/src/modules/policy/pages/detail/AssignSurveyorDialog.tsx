...
export default function AssignSurveyorDialog({ open, onOpenChange, policyId, policyNumber, onSuccess }: Props) {
  ...
  const assign = useMutation({
    mutationFn: async (values: FormValues) => {
      ...
      const res = await apiClient.post<{ data: unknown }>(
        `/api/v1/policies/${policyId}/survey/assign`,
        {
    ...
    onError: (e) => applyApiErrors(e, form, { defaultTitle: 'Could not assign surveyor' }),
  });
    ...
    <Dialog open={open} onOpenChange={(v) => { if (!v) form.reset({ surveyorType: 'INTERNAL', surveyorId: '' }); onOpenChange(v); }}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Assign a surveyor for the pre-loss inspection on <span className="font-medium text-foreground">{policyNumber}</span>.
          </DialogDescription>
        </DialogHeader>
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            <DialogFooter>
              ...
              <Button type="submit" disabled={assign.isPending || surveyors.length === 0}>
                {assign.isPending ? 'Assigning…' : 'Assign Surveyor'}
              </Button>
            </DialogFooter>
          </form>
        </Form>
